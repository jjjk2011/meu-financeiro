            ...
            <tr class="${t.pago ? 'opacity-30' : ''} border-b dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                <td class="py-4 px-2 w-8">
                    <button onclick="togglePago('${t.id}')" class="w-5 h-5 rounded-full border-2 transition-all transform hover:scale-110 ${
                        t.pago ? 'bg-emerald-500 border-emerald-500' : 'border-slate-300 hover:border-emerald-500'
                    }"></button>
                </td>
                ...
                <td class="text-right px-2">
                    <button onclick="excluir('${t.id}')" class="text-slate-300 hover:text-rose-500 hover:scale-110 transition-all text-xs font-bold">
                        ✕
                    </button>
                </td>
            </tr>`).join('');
...
function initKeyboardShortcuts() {
    document.addEventListener('keydown', (e) => {
        ...
        // Ctrl/Cmd + S: Salvar (se estiver logado)
