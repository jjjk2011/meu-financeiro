...
async function handleLogin() {
    ...
    catch (err) { alert("Erro de login: Verifique e-mail e senha."); }
}
...
async function handleSignup() {
    ...
    try { 
        await window.fb_funcs.createUserWithEmailAndPassword(window.auth, e, p);
...
// --- ADICIONAR ITENS NAS LISTAS ---
async function promptNovoItem(tipo) {
    const novo = prompt(`Digite o novo nome para ${tipo}:`);
    if (novo) {
...
function render() {
    ...
    if(!fMonthEl || !fYearEl) return;

    const mIdx = MESES.indexOf(fMonthEl.value);
    const yVal = parseInt(fYearEl.value);
